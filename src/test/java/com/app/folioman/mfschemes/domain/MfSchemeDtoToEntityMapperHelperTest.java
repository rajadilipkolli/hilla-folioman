...
class MfSchemeDtoToEntityMapperHelperTest {
    ...
    private MfAmcEntity mockAmc;
    private MFSchemeTypeEntity mockSchemeType;

    @BeforeEach
    void setUp() {
        mockAmc = new MfAmcEntity();
        mockAmc.setId(1);
        mockAmc.setName("Test AMC");

        mockSchemeType = new MFSchemeTypeEntity();
        ...
        mfScheme = new MfFundSchemeEntity();
        mfScheme.setAmc(new MfAmcEntity());
    }
    ...
    void updateMFScheme_WithValidNavAndComplexSchemeType_ShouldMapCorrectly() {
        ...
        assertThat(mfScheme.getMfSchemeTypeEntity()).isEqualTo(mockSchemeType);
        assertThat(mfScheme.getAmc()).isEqualTo(mockAmc);
        assertThat(mfScheme.getMfSchemeNavs()).isNotEmpty();
        MFSchemeNavEntity nav = mfScheme.getMfSchemeNavs().getFirst();
    ...
    void updateMFScheme_WithInvalidSchemeTypeContainingDash_ShouldLogError() {
        ...
        assertThat(mfScheme.getMfSchemeTypeEntity()).isNull();
    ...
    void findOrCreateMFSchemeTypeEntity_WhenExistsInDatabase_ShouldReturnExisting() {
        ...
        MFSchemeTypeEntity result = mapperHelper.findOrCreateMFSchemeTypeEntity("Equity", "Large Cap", "Growth");
    ...
    void findOrCreateMFSchemeTypeEntity_WhenNotExistsInDatabase_ShouldCreateNew() {
        when(mFSchemeTypeService.findByTypeAndCategoryAndSubCategory(eq("Equity"), eq("Mid Cap"), eq("Value")))
                .thenReturn(null);
        when(mFSchemeTypeService.saveCategory(any(MFSchemeTypeEntity.class))).thenReturn(mockSchemeType);

        MFSchemeTypeEntity result = mapperHelper.findOrCreateMFSchemeTypeEntity("Equity", "Mid Cap", "Value");

        assertThat(result).isEqualTo(mockSchemeType);
        verify(mFSchemeTypeService).findByTypeAndCategoryAndSubCategory("Equity", "Mid Cap", "Value");
        verify(mFSchemeTypeService).saveCategory(any(MFSchemeTypeEntity.class));
    }
    ...
    void findOrCreateMFSchemeTypeEntity_WithNullSubCategory_ShouldHandleCorrectly() {
        ...
        MFSchemeTypeEntity result = mapperHelper.findOrCreateMFSchemeTypeEntity("Debt", "Long Term", null);
    ...
    void findOrCreateMFSchemeTypeEntity_ShouldUseCache_OnSecondCall() {
        ...
        MFSchemeTypeEntity result1 = mapperHelper.findOrCreateMFSchemeTypeEntity("Equity", "Large Cap", "Growth");
        MFSchemeTypeEntity result2 = mapperHelper.findOrCreateMFSchemeTypeEntity("Equity", "Large Cap", "Growth");
